...
const getTransactionsOfCustomer = async (customerId, req, res) => {
  ...
  if (data && data.length) {
    const body = {data: data, status: 200, customerBalance: customer.balance, message: "Transactions fetched successfully"};
...
const convertToCsv = (data) => {
  ...
  const headers = ["createdAt", "customerName", "debit", "credit"];
  //   csvRows.push(headers.join(','));
  //   console.log('csvRowscsvRows', csvRows);
  const csvRows = ["Date,Customer Name,Debit,Credit"];
...
const modifyCustomerBalance = async (transaction, params) => {
  ...
  let newBal = customer.balance;

  if (params) {
    if (params.amount) {
      if (transaction.type === "DEBIT") {
        newBal = customer.balance + transaction.amount - params.amount;
      } else {
        newBal = customer.balance - transaction.amount + params.amount;
      }
    }
    if (params.type && transaction.type !== params.type) {
      ...
      if (transaction.type === "DEBIT") {
        newBal = customer.balance + transaction.amount + amt;
      } else {
        newBal = customer.balance - transaction.amount - amt;
      }
    }
    ...
    let customerBal = customer.balance - transaction.amount;
    ...
    if (customer.balance === 0) {
      ...
      customerBal = customer.balance - transAmt;
    }
